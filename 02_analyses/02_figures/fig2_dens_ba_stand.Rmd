---
title: "Quantify the relationship between changes in species abundance and mean tree size adjusting for stand development"
...
species_ba_changes <- data_model_chan |> 
  group_by(species.cor) |> 
  summarise(
    mean_sp_mean_ba_ind0 = mean(
      mean.ba.ind0, na.rm = T
      ),
    mean_sp_mean_ba_ind = mean(
      mean.ba.ind, na.rm = T
      ),
...
mutate_name <- function(df, names_stand) {
  df |> 
    dplyr::mutate(
      stand_dev = names_stand
    )
}

names_stand_v <- c(
  "All data", "Early stand development",
  "Late stand development"
)

arg2_df_names_stand <- list(
  df = data_ls,
  names_stand = names_stand_v
)

data_ls_m <- arg2_df_names_stand |> 
  pmap(mutate_name)
...
# late
mean_late_n_plot_changes <- mean(data_ls_m[[3]]$n.plot_changes, na.rm = T)
mean_late_ba_ind_changes_cm <- mean(data_ls_m[[3]]$mean.ba.ind_changes, na.rm = T) / 100
...
gg_stand_dev <- ggplot(
  ...
  geom_text_repel(max.overlaps = Inf, 
                  ...
                  fontface = "italic") +
  scale_y_continuous(
    name = (bquote("Annual change mean tree basal area  "(cm^2)))
  ) +
  scale_x_continuous(
    name = "Annual change n. stems per hectare",
  ) +
  facet_grid(. ~ stand_dev) +
...
ggsave(
  plot = gg_stand_dev,
  here("03_results", "figures", "fig2", "fig2_main.png"),
  width = 8, height = 4,
  dpi = 600
)
